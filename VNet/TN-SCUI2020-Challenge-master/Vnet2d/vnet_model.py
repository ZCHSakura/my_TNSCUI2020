...
class Vnet2dModule(object):
    ...
    def __get_accuracy(self, Y_gt, Y_pred, prob=0.8):
        ...
        union = tf.reduce_sum(pred_flat, axis=-1) + tf.reduce_sum(true_flat, axis=-1) - intersection
        metric = tf.reduce_mean((intersection + smooth_tf) / (union + smooth_tf))
        # 进行分支判断，如果真，smooth_tf;如果假，metric
        metric = tf.cond(tf.is_inf(metric), lambda: smooth_tf, lambda: metric)
        return metric

    def train(self, train_images, train_lanbels, model_name, logs_path, learning_rate,
              dropout_conv=0.5, train_epochs=10, batch_size=1, restore=False, step=0):
        if not os.path.exists(logs_path):
            os.makedirs(logs_path)

        # 记录config
        f = open(os.path.join(logs_path, 'config.txt'), 'w')
        print('%s: %s' % ('image_width', self.image_width), file=f)
        print('%s: %s' % ('image_height', self.image_height), file=f)
        print('%s: %s' % ('channels', self.channels), file=f)
        print('%s: %s' % ('model_name', model_name), file=f)
        print('%s: %s' % ('logs_path', logs_path), file=f)
        print('%s: %s' % ('alpha', self.alpha), file=f)
        print('%s: %s' % ('gamma', self.gamma), file=f)
        print('%s: %s' % ('costname', self.costname), file=f)
        print('%s: %s' % ('learning_rate', learning_rate), file=f)
        print('%s: %s' % ('dropout_conv', dropout_conv), file=f)
        print('%s: %s' % ('train_epochs', train_epochs), file=f)
        print('%s: %s' % ('batch_size', batch_size), file=f)
        print('%s: %s' % ('restore', restore), file=f)
        print('%s: %s' % ('step', step), file=f)
        f.close()
        ...
        if restore == True:
            restor_model_path = model_path.replace('.pd', '.pd-{}'.format(step))
            print(restor_model_path)
            saver.restore(sess, restor_model_path)

        DISPLAY_STEP = 1
        index_in_epoch = 0

        print(train_images.shape)
        train_epochs = train_images.shape[0] * train_epochs - step
        for i in range(train_epochs):
            # get new batch
            batch_xs_path, batch_ys_path, index_in_epoch = _next_batch(train_images, train_lanbels, batch_size,
                                                                       index_in_epoch)
            ...
            if i % DISPLAY_STEP == 0 or (i + 1) == train_epochs:
                ...
                cv2.imwrite(true_path, true)
                print('epochs %d training_loss ,Training_accuracy => %.5f,%.5f ' % (i, train_loss, train_accuracy))
                save_path = saver.save(sess, model_path, global_step=i+step)
                print("Model saved in file:", save_path)
                if i % (DISPLAY_STEP * 10) == 0 and i:
                    DISPLAY_STEP *= 10
            # train on batch
            _, summary = sess.run([train_op, merged_summary_op], feed_dict={self.X: batch_xs,
                                                                            ...
                                                                            self.phase: 1,
                                                                            self.drop_conv: dropout_conv})
            summary_writer.add_summary(summary, i+step)
        summary_writer.close()
        save_path = saver.save(sess, model_path)
        print("Model saved in file:", save_path)
