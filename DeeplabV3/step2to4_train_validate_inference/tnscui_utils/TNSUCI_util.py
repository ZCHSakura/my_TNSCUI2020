...
def get_fold_filelist(csv_file, K=3, fold=1, random_state=2020, validation=False, validation_r = 0.2, test_csv = None):
    """
    ...
    if validation is False:     # 不设置验证集，则直接返回
        ...
        ws_tarin = wb["train"]
        ws_test = wb["test"]
        ws_tarin.cell(1, 1).value = 'ID'
        ws_test.cell(1, 1).value = 'ID'
        for train_id in train_set:
            idd = (train_id[0].split('.')[0], )
            ws_tarin.append(idd)  # 每次写入一行
        for test_id in test_set:
            iddd = (test_id[0].split('.')[0], )
            ws_test.append(iddd)  # 每次写入一行
        wb.save(os.path.join(test_csv, 'train_test_id.xlsx'))
        ...
        ws_test = wb["test"]
        ws_tarin.cell(1, 1).value = 'ID'
        ws_valid.cell(1, 1).value = 'ID'
        ws_test.cell(1, 1).value = 'ID'
        for train_id in train_set:
            idd = (train_id[0].split('.')[0],)
            ws_tarin.append(idd)  # 每次写入一行
        for valid_id in validation_set:
            iddd = (valid_id[0].split('.')[0],)
            ws_valid.append(iddd)  # 每次写入一行
        for test_id in test_set:
            idddd = (test_id[0].split('.')[0],)
            ws_test.append(idddd)  # 每次写入一行
        wb.save(os.path.join(test_csv, 'train_valid_test_id.xlsx'))
