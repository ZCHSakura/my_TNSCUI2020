...
from step2to4_train_validate_inference.loss_func.dice_helpers import soft_cldice_loss
from step2to4_train_validate_inference.loss_func.dice_loss import FocalTversky_loss
from step2to4_train_validate_inference.loss_func.lovasz_losses import lovasz_hinge, binary_xloss
from step2to4_train_validate_inference.tnscui_utils.TNSUCI_util import char_color, GradualWarmupScheduler, sep
from step2to4_train_validate_inference.utils.evaluation import *
from step2to4_train_validate_inference.utils.misc import printProgressBar
...
class Solver(object):
	...
	def build_model(self):
		...
		if os.path.isfile(unet_path):
			print('optimizer with best_unet')
			checkpoint = torch.load(unet_path, map_location='cpu')
			last_lr = checkpoint['lr']
			print('----', last_lr)
			self.lr = last_lr
			# 修改后面的lr策略epoch数
			self.lr_cos_epoch = self.num_epochs - checkpoint['epoch']
			self.lr_warm_epoch = 0
			print(self.lr_cos_epoch, self.lr_warm_epoch)
		# 优化器修改
	...
	def train(self):
		...
		if os.path.isfile(unet_path):	 # False:
			...
			last_lr = checkpoint['lr']
			self.myprint('last_epoch: %s' % (last_epoch))
			self.myprint('last_lr: %s' % (last_lr))
		...
		# 控制record.xlsx的startrow
		df_rows = 0
		if reload_flag:
			df_last = pd.DataFrame(pd.read_excel(os.path.join(self.result_path, 'record.xlsx'), sheet_name='valid'))  # 读取原数据文件和表
			df_rows = df_last.shape[0]  # 获取原数据的行数
		...
		for epoch in range(last_epoch, self.num_epochs):
			...
			self.myprint('current_lr: %s' % (current_lr))
			...
			writer.add_scalars('Learning rate', {'lr': current_lr}, epoch + 1)
			# 保存lr为png
			figg = plt.figure()
			...
			if (epoch + 1) % self.val_step == 0:
				...
				unet_score = IOU # TODO
				writer.add_scalars('Valid', {'Dice': DC, 'IOU': IOU}, epoch + 1)
				...
				if unet_score > best_unet_score:
					...
					checkpoint = {
						'epoch': best_epoch,
						'state_dict': best_unet,
						'Iter': Iter,
						'lr': current_lr,
					}
					print("save_best_epoch", best_epoch)
				...
				if self.test_flag:
					...
					writer.add_scalars('Test', { 'Dice': DC, 'IOU': IOU}, epoch + 1)
				...
				if (True):
					...
					# 	record0 = pd.ExcelWriter(excel_save_path, engine='openpyxl')
					# 	detail_result0 = pd.DataFrame(valid_record)
					# 	detail_result0.to_excel(record0, 'valid', float_format='%.5f')
					# 	print(1)
					# 	record0.save()
					# 	record0.close()
					# else:
					# 	print(2)
					record = pd.ExcelWriter(excel_save_path, engine='openpyxl')
					detail_result1 = pd.DataFrame(valid_record)
					if reload_flag:
						print(1)
						book = load_workbook(excel_save_path)
						record.book = book
						record.sheets = dict((ws.title, ws) for ws in book.worksheets)
						print("df_rows1:", df_rows)
						detail_result1.to_excel(record, 'valid', float_format='%.5f', startrow=df_rows+1)
					else:
						print(2)
					...
					if self.test_flag:
						...
						if reload_flag:
							print(3)
							print("df_rows2:", df_rows)
							detail_result2.to_excel(record, 'test', float_format='%.5f', startrow=df_rows+1)
						else:
							print(4)
			...
			if (epoch+1) % self.save_model_step == 0:
				...
				torch.save(checkpoint,os.path.join(self.model_path, 'epoch%d_Testdice%.4f.pkl'%(epoch+1, DC)))
